...
const getToken = () => localStorage.getItem("authToken");
 ...
 const BatchModuleScheduleTable = ({ editable }: BatchModuleScheduleTableProps) => {  
  ...
  const validateFields = () => {
    ...
    if (!newSchedule.batchName) newErrors.batchName = "Batch name is required.";
    if (!newSchedule.moduleName) newErrors.moduleName = "Module name is required.";
    if (!newSchedule.trainerName) newErrors.trainerName = "trainer name is required.";
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      console.log('schedulesbatches', schedules);
    ...
    const editSchedule = (data: any) => {
      ...
      if (scheduleToEdit) {
        ...
        console.error("Batch not found for editing.");
    ...
    const handleFormSubmit = async () => {
      ...
      try {
        if (editing) {
          ...
          setSchedules((prev) =>
            prev.map((schedule) =>
              schedule.id === newSchedule.id ? { ...schedule, ...updateSchedule } : schedule
            )
          );
      ...
      setColDefs([
        ...
        { headerName: "Duration", field: "duration", editable: false },
        {
          ...
          cellRenderer: (params: any) => {
            console.log("Editing schedule data:", params.data);
            return (
              <div className="flex space-x-2">
                <Button onClick={() => editSchedule(params)} className="bg-blue-500 text-white p-2 rounded hover:bg-blue-700">
    ...
    const uniqueTrainer = Array.from(
      new Map(schedules.map((schedule) => [schedule.trainerName, schedule.trainerId]))
    );
              ...
              <form>
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewSchedule({
                        ...newSchedule, trainerId: parseInt(e.target.value),
                        trainerName:
                          uniqueTrainer.find(([_, id]) => id === parseInt(e.target.value))?.[0] || "",
                      })
                    }
                  ...
                  >
                    ...
                    {trainers.map((trainer) => (
                      <option key={trainer.trainerName} value={trainer.id}>
                        {trainer.trainerName}
                      </option>
                    ))}
                  </select>
